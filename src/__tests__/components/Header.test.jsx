...
describe('Header', () => {
  ...
  it('총 평가액 표시 (KRW)', () => {
    renderHeader({ ...defaultProps, totalVal: 17000000, totalCost: 10000000, pl: 7000000, ret: 70, displayCurrency: 'KRW' })
  ...
  it('환율 있으면 토글 버튼 표시', () => {
    renderHeader({ ...defaultProps, exchangeRate: { rate: 1380, updatedAt: new Date().toISOString() } })
    expect(screen.getByText('USD')).toBeInTheDocument()
    expect(screen.getByText('KRW')).toBeInTheDocument()
  })

  it('환율 없으면 토글 버튼 숨김', () => {
    renderHeader({ ...defaultProps, exchangeRate: { rate: null, updatedAt: null } })
    expect(screen.queryByText('KRW')).not.toBeInTheDocument()
  })

  it('토글 클릭 시 onToggleCurrency 호출', () => {
    const onToggle = vi.fn()
    renderHeader({ ...defaultProps, exchangeRate: { rate: 1380, updatedAt: new Date().toISOString() }, onToggleCurrency: onToggle })
  ...
  it('☰ 버튼 클릭 시 onMenuOpen 호출', () => {
    ...
    renderHeader({ ...defaultProps, onMenuOpen })
